...
public class BoardManageService {
    ...
    private static final Logger logger = LoggerFactory.getLogger(BoardManageService.class);
    ...
    public List<BoardList> getBoardList() {
        logger.error("resultMap :: {}", boardListRepository.findAll());
    ...
    public boolean createBoardTable(BoardListDto boardListDto) {
        logger.error("boardListDto.getBoardTitle :: {}", boardListDto.toString());
        ...
        if(iResult > 0){
            ...
            if(iResult > 0){
                ...
                if(tempTagList.isEmpty() == false){
                    iResult = insertTagList(boardId, tempTagList);
                    ...
                    logger.error("등록할 태그 없음");
                    return true;
                }
            }
            else{
                logger.error("메뉴등록 실패");
                return false;
            }
        }
        else 
    ...
    int insertMenuList(int boardId, BoardListDto boardListDto) {
        ...
        if(boardListDto.getBoardRole().equals("ROLE_ADMIN"))
        {
         ...
         {
             menuDto.setMenuRole("888");
         }
         else { 
             menuDto.setMenuRole("999");
         }
         ...
         logger.error("menuDto :: {}", menuDto);
         return menuMapper.insertMenuData(menuDto);
    }
    
    int insertTagList(int boardId, String tempTagList) {
        ...
        {
            TagDto ld = new TagDto(i+1, array[i], boardId);
            //ld.setTagId(i+1);
    ...
    boolean deleteAllAfterProc(HashMap<String, Object> hm) {
        ...
        if(iResult > 0){
            ...
            logger.error("deleteBoardList iResult :: {}", iResult);
